...
func getOrFetchCompleteAlbumData(albumName, artistName, albumDir string) (AlbumData, bool) {
	...
	pdk.Log(pdk.LogInfo, fmt.Sprintf("⏳ [%s - %s] -> (推导物理路径: %s)", artistName, albumName, albumDir))
...
func resolveID(query string, searchType int) (int64, string, error) {
	...
	if cached, ok := Storage.GetIDMap(searchType, targetName); ok {
	...
	if searchType == 100 && len(sr.Result.Artists) > 0 {
		...
		if len(exactMatches) > 1 {
			...
			if len(exactMatches) > 10 {
				pdk.Log(pdk.LogWarn, fmt.Sprintf("⚠️ [OOM 保护] 词条 [%s] 同名冲突 %d 个，提取前 10 个", targetName, len(exactMatches)))
				exactMatches = exactMatches[:10]
			}
	...
	if foundID != 0 {
		Storage.SaveIDMap(searchType, targetName, foundID, foundPic)
	}
...
func fetchAndWriteLocalLyrics(title, artist, album, absolutePath string, knownSongID int64) string {
	...
	songID := knownSongID
	var localData AlbumData
	var found bool

	if absolutePath != "" {
		albumDir := getCleanAlbumDir(absolutePath)
		localData, found = getLocalAlbumData(albumDir)
	...
	if songID == 0 && title != "" {
		...
		sid, _, _ := resolveID(query, 1)
		songID = sid
		if songID > 0 {
			pdk.Log(pdk.LogInfo, fmt.Sprintf("🔍 歌词动作: 无绝对路径或未匹配本地 JSON，通过 API 强搜命中单曲 ID: %d", songID))
		}
	}
	...
	pdk.Log(pdk.LogInfo, fmt.Sprintf("📄 歌词动作: 请求官方接口拉取歌词 (SongID: %d)", songID))
	...
	if absolutePath != "" && lrcPath != "" && getConfigBool("enable_write_lyrics", true) {
